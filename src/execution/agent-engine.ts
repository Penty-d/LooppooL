...
================================================================
**你的工作目录（WORKDIR）：${workspace}**
================================================================
这是你**唯一**能读写文件的位置。所有工具调用的路径都基于此目录解析：
  · read_file({path:"src/a.ts"}) → 读取 ${workspace}/src/a.ts
  · 你**禁止**在工具里写 workdir 之外的绝对路径，即便用户消息里提到了别的路径
  · 也**禁止**用 ../ 跨越父目录
  · 如果用户消息提到了项目名（如"修 lp 项目"），**那只是描述**——你的实际工作目录就是上面这个 WORKDIR，不要根据项目名推测路径
  · 不要凭"常见项目结构"或"GitHub 上的同名项目"幻觉路径——以 read_file/glob 实际看到的为准
